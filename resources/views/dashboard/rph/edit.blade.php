...
<form action="{{ route('dashboard.rph.update', $rph->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="row">
        <div class="col-xl-8 col-lg-7">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="form-group">
                        <input type="text" class="form-control" name="title" value="{{ old('title', $rph->title) }}" />
                        @error('title')
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <textarea id="content-rph" name="content">{{ old('content', $rph->content) }}</textarea>
                    @error('content')
                    <small class="form-text error-input">{{ $message }}</small>
                    @enderror
                </div>
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <div class="custom-control ">
                            <select class="form-control" name="sub_menus_id">
                                ...
                                <option value="{{$item->id}}" @if($rph->sub_menus_id == $item->id) selected @endif>{{ old('name', $item->name) }}</option>
                                @endforeach
                            </select>
                            @error('sub_menus_id')
                            <small class="form-text error-input">{{ $message }}</small>
                    ...
                    <div>
                        ...
                        <div class="card my-2">
                            <label for="imageUpload" class="mb-0 cursor-pointer">
                                @if(!empty($rph->featured_image))
                                <img id="image-preview" class="card-img-top" src="{{ $rph->featured_image }}" alt="Card image cap">
                                <input type="hidden" name="featured_image" value="{{ $rph->featured_image }}" />
                                @else
