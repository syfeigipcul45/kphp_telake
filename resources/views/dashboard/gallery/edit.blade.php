                        ...
                        <div class="col-md-6">
                            ...
                            <div class="card my-2">
                                <label for="imageUpload" class="mb-0 cursor-pointer">
                                    @if(!empty($gallery->getFirstMediaUrl('galleries', 'thumb')))
                                    <img id="image-preview" class="card-img-top" src="{{ $gallery->getFirstMediaUrl('galleries', 'thumb') }}" alt="Card image cap">
                                    <input type="hidden" name="thumbnail" value="{{ $gallery->getFirstMediaUrl('galleries', 'thumb') }}" />
                                    @else
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="caption" value="{{ old('caption', $gallery->caption) }}" />
                                @error('caption')
                                <small class="form-text error-input">{{ $message }}</small>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="kategori_id" id="category-id" class="form-control">
                                    <option disabled selected value="">:: Pilih Kategori Foto</option>
                                    @foreach($categories as $item)
                                    <option value="{{ $item->id }}" @if($gallery->kategori_id === $item->id) selected @endif>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                @error('kategori_id')
                                <small class="form-text error-input">{{ $message }}</small>
